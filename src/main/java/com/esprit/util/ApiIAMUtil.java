...
public class ApiIAMUtil {

	private static final Logger log = Logger.getLogger(ApiIAMUtil.class);
	...
	public static String getCreatedId(Response response) {
		...
		if (!response.getStatusInfo().equals(Status.CREATED)) {
			...
			throw new WebApplicationException(
					"Create method returned status " + statusInfo.getReasonPhrase() + " (Code: "
							+ statusInfo.getStatusCode() + "); expected status: Created (201). Response body: " + body,
					response);
		}
		if (location == null) {
			return null;
	 ...
	 * @return ID of the new user
	 */
	public static String createUserAndResetPasswordWithAdminClient(RealmResource realm, UserRepresentation user,
			String password) {
	...
	public static void assignClientRoles(RealmResource realm, String userId, String clientName, String... roles) {
		...
		if (!clientId.isEmpty()) {
			...
			log.info("assigning role: " + Arrays.toString(roles) + " to user: \""
					+ userResource.toRepresentation().getUsername() + "\" of client: \"" + clientName
					+ "\" in realm: \"" + realmName + "\"");
